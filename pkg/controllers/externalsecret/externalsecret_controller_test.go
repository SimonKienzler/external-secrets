...
var _ = Describe("ExternalSecret controller", func() {
	...
	BeforeEach(func() {
		var err error
		ExternalSecretNamespace, err = ctest.CreateNamespaceWithLabels("test-ns", k8sClient, map[string]string{"eso": "true"})
	...
	shouldSkipNamespaceWhenSelectorDoesNotMatch := func(tc *testCase) {
		...
		Expect(r.ShouldSkipNamespace(tc.secretStore, ExternalSecretNamespace)).To(BeTrue())

		tc.checkCondition = func(es *esv1beta1.ExternalSecret) bool {
			cond := GetExternalSecretCondition(es.Status, esv1beta1.ExternalSecretDeleted)
			return cond == nil
		}
	}
